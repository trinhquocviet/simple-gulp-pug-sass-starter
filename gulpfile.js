...
gulp.task('setup-src', function() {
  ...
  if(data[data.length - 1].indexOf('script(src="js/bundle.js")') > -1) {
    data.pop();
  }
  ...
  if(!findKeyText(data, 'bundle.js')) {
    data.splice(data.length, 0, '    script(src="js/bundle.js")')
  }

  var text = data.join("\n");
  fs.writeFile('./src/index.pug', text, function (err) {
    if (err) throw err;
  });
});
...
gulp.task('watch', function() {
  ...
  gulp.watch(['src/templates/**/*', './src/*.pug'],    ['templates']);
